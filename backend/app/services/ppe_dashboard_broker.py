"""
PPE Dashboard Broker — in-memory per-project asyncio.Queue registry for SSE delivery.

Keyed by project_id (not user_id) so every browser tab viewing a project dashboard
receives real-time ppe_live_alert and ppe_stats_update events regardless of which
user account is logged in — as long as they have an open SSE connection.

Thread-safe: push() is called from the incident queue worker thread.
...
def set_event_loop(loop: asyncio.AbstractEventLoop) -> None:
    ...
    global _loop
    _loop = loop
    logger.info("[ppe_dashboard_broker] Event loop registered")
...
def register(project_id: int) -> asyncio.Queue:
    ...
    logger.debug(f"[ppe_dashboard_broker] Registered queue for project {project_id}")
    return q
...
def unregister(project_id: int, q: asyncio.Queue) -> None:
    ...
    logger.debug(f"[ppe_dashboard_broker] Unregistered queue for project {project_id}")
