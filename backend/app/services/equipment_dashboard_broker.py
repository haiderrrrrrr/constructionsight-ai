"""
Equipment Dashboard Broker — in-memory per-project asyncio.Queue registry for SSE delivery.

Keyed by project_id so every browser tab viewing a project's equipment dashboard
receives real-time equipment_stats_update and equipment_alert events.

Thread-safe: push() is called from EquipmentProcessor consumer threads.
...
def set_event_loop(loop: asyncio.AbstractEventLoop) -> None:
    ...
    global _loop
    _loop = loop
    logger.info("[equipment_dashboard_broker] Event loop registered")
...
def register(project_id: int) -> asyncio.Queue:
    ...
    logger.debug(f"[equipment_dashboard_broker] Registered queue for project {project_id}")
    return q
...
def unregister(project_id: int, q: asyncio.Queue) -> None:
    ...
    logger.debug(f"[equipment_dashboard_broker] Unregistered queue for project {project_id}")


def push(project_id: int, payload: dict) -> None:
    """
    Push a payload to all active SSE connections for project_id.
    Thread-safe — safe to call from EquipmentProcessor threads.
