"""
Project Task Broker — in-memory per-project asyncio.Queue registry for SSE delivery.

Keyed by project_id so every browser tab viewing a project's tasks page receives
real-time task_created events when auto_task_engine creates tasks after incidents,
without requiring a manual page refresh.

Thread-safe: push() is called from background incident processing threads.
...
def set_event_loop(loop: asyncio.AbstractEventLoop) -> None:
    ...
    global _loop
    _loop = loop
    logger.info("[project_task_broker] Event loop registered")
...
def register(project_id: int) -> asyncio.Queue:
    ...
    logger.debug(f"[project_task_broker] Registered queue for project {project_id}")
    return q
...
def unregister(project_id: int, q: asyncio.Queue) -> None:
    ...
    logger.debug(f"[project_task_broker] Unregistered queue for project {project_id}")
