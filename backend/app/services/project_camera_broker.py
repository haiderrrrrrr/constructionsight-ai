"""
Project Camera Broker — in-memory per-project asyncio.Queue registry for SSE delivery.

Keyed by project_id so every browser tab viewing a project's camera list or
camera detail page receives real-time camera_health_update and
camera_verification_update events without polling.
...
def set_event_loop(loop: asyncio.AbstractEventLoop) -> None:
    ...
    global _loop
    _loop = loop
    logger.info("[project_camera_broker] Event loop registered")
...
def register(project_id: int) -> asyncio.Queue:
    ...
    logger.debug(f"[project_camera_broker] Registered queue for project {project_id}")
    return q
...
def unregister(project_id: int, q: asyncio.Queue) -> None:
    ...
    logger.debug(f"[project_camera_broker] Unregistered queue for project {project_id}")
