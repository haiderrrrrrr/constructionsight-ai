/**
 * Shared React Query cache patch helpers for the Activity Monitoring dashboard.
...
export function patchAlertInCache(queryClient, projectId, data) {
    queryClient
        .getQueriesData({ queryKey: ['activity', 'alerts', projectId] })
        .forEach(([key, cached]) => {
    ...
    if (data.open_alerts != null) {
        queryClient
            .getQueriesData({ queryKey: ['activity', 'summary', projectId] })
            .forEach(([key, cached]) => {
                if (!cached) return
