/**
 * Shared React Query cache patch helpers for the Workforce Analytics dashboard.
 * Mirrors ppeCacheUtils.js — surgical in-place updates without triggering full
...
export function patchAlertInCache(queryClient, projectId, data) {
    queryClient
        .getQueriesData({ queryKey: ['workforce', 'alerts', projectId] })
        .forEach(([key, cached]) => {
    ...
    if (data.open_alerts != null) {
        queryClient
            .getQueriesData({ queryKey: ['workforce', 'summary', projectId] })
            .forEach(([key, cached]) => {
                if (!cached) return
