/**
 * activity-analytics.jsx — Activity Monitoring page
 *
 * Live project activity dashboard:
 *  - Project selector in PageHeader (global route only)
 *  - ActivityDashboard (all charts, KPI cards, trend, alerts)
 */
...
import ActivityDashboard from '@/components/projectWorkspace/ActivityDashboard'
...
export default function ActivityAnalytics() {
    ...
    const { data: actStatusPage } = useQuery({
        queryKey: QK.actStatus(pid),
        queryFn: () => apiGet(`/projects/${pid}/cameras/features`).catch(() => null),
        staleTime: 30_000,
        enabled: !!pid,
    })
    const featureActive = useMemo(() => {
        const cams = actStatusPage?.cameras ?? (Array.isArray(actStatusPage) ? actStatusPage : [])
        return cams.some(c => c?.features?.activity_enabled === true)
    }, [actStatusPage])

    const actServerLiveStart = useMemo(() => (
        actStatusPage?.activity_live_session_start ??
        actStatusPage?.live_session_start ??
        null
    ), [actStatusPage])

    const handleExport = async () => {
        if (exporting || !selectedProject) return
        if (isLive && featureActive && !actServerLiveStart) {
            topTostError('Activity live session start is not available yet. Please wait a moment and try again.')
            return
        }
        setExporting(true)
        try {
            const token = window.sessionStorage.getItem('access_token')
            const exportFrom = isLive ? new Date(actServerLiveStart) : dateFilter.from
            ...
            a.href     = url
            a.download = `Activity_Monitoring_Report_${label}.pdf`
            document.body.appendChild(a); a.click(); document.body.removeChild(a)
            window.URL.revokeObjectURL(url)
            window.dispatchEvent(new Event('cs:report-status-changed'))
            topTostError('Activity Monitoring Report downloaded successfully.', 'success')
        } catch (err) {
                    ...
                    <div className="card stretch stretch-full">
                        <div className="card-body d-flex flex-column align-items-center justify-content-center py-5 text-muted gap-3">
                            ...
                            <div className="text-center">
                                ...
                                <div className="fs-12">
                                    Activity Monitoring is available to Project Managers, Site Supervisors, and Safety Officers on active projects.
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <>
            <PageHeader projectCrumbsKey={isProjectScoped ? "reports" : undefined} projectCrumbsLeaf={isProjectScoped ? "activity" : undefined}>
                <div className="d-flex align-items-center gap-2 page-header-right-items-wrapper">
                    ...
                    <PageHeaderDate
                        ...
                        exportDisabledTitle="Enable Activity Monitoring on at least one camera to generate reports"
                        liveMode={isLive}
                        onLiveSelect={() => {
            ...
            {!isProjectScoped && selectedProject && (
                <>
                    ...
                    <ActivityLiveAlertToasts projectId={selectedProject.id} />
                </>
            )}
