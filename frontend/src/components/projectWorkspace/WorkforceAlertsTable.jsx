...
const TYPE_MAP = {
    understaffed:    { label: 'Understaffed',    color: 'danger' },
    idle_ratio_high: { label: 'High Idle Ratio', color: 'warning' },
    sudden_drop:     { label: 'Sudden Drop',     color: 'danger' },
    overload:        { label: 'Zone Overload',   color: 'danger' },
}
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function WorkforceAlertsTable({
    projectId,
    cameraId,
    dateFrom,
    dateTo,
    ...
    const qKey = QK.wfAlerts(projectId, page, dateFrom, dateTo, null, cameraId)
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const dateParam = dateFrom && liveTo
                ...
                : ''
            return apiGet(`/projects/${projectId}/workforce/alerts?page=${page}&per_page=${PER_PAGE}${camParam}${dateParam}`)
        },
        staleTime: 30_000,
    ...
    // Fallback: full refetch when a new alert arrives (workforce_alert path).
    ...
    useEffect(() => {
        ...
        window.addEventListener('cs:workforce-alerts-refresh', handler)
        return () => window.removeEventListener('cs:workforce-alerts-refresh', handler)
    }, [projectId, queryClient])

    const statusMutation = useMutation({
        mutationFn: async ({ alertId, newStatus }) => {
            return apiPatch(`/projects/${projectId}/workforce/alerts/${alertId}/status`, { status: newStatus })
        },
        ...
        onSuccess: (_data, { alertId, newStatus }) => {
            ...
            broadcastRefresh('wf:alert-updated', { projectId, alert_id: alertId, status: newStatus })
        ...
        <>
            ...
            <div className="card stretch stretch-full">
                <div className="card-header">
                    <div>
                        <h5 className="mb-0">Workforce Insights</h5>
                        <span className="fs-12 text-muted">Zone-level workforce activity and staffing insights across monitored camera</span>
                    </div>
                </div>
