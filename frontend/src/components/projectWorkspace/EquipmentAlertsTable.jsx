...
const TYPE_MAP = {
    idle_waste:           { label: 'Idle Waste',           color: 'warning' },
    active_no_workers:    { label: 'Active – No Workers',  color: 'danger' },
    ghost_equipment:      { label: 'Ghost Equipment',      color: 'danger' },
    overuse:              { label: 'Overuse',              color: 'danger' },
    cross_zone_conflict:  { label: 'Cross-Zone Conflict',  color: 'danger' },
}
...
const SnapshotThumb = ({ url, onClick }) => {
    if (!url) {
        return (
            <button
                type="button"
                className="avatar-text avatar-md eq-eye-disabled"
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function EquipmentAlertsTable({
    projectId,
    cameraId,
    dateFrom,
    dateTo,
    ...
    const qKey = QK.eqAlerts(projectId, page, dateFrom, dateTo, null, cameraId)
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const dateParam = dateFrom && liveTo
                ...
                : ''
            return apiGet(`/projects/${projectId}/equipment/alerts?page=${page}&per_page=${PER_PAGE}${camParam}${dateParam}`)
        },
        staleTime: 30_000,
    ...
    useEffect(() => {
        ...
        window.addEventListener('cs:equipment-alerts-refresh', handler)
        return () => window.removeEventListener('cs:equipment-alerts-refresh', handler)
    }, [projectId, queryClient])

    const statusMutation = useMutation({
        mutationFn: async ({ alertId, newStatus }) => {
            return apiPatch(`/projects/${projectId}/equipment/alerts/${alertId}/status`, { status: newStatus })
        },
        ...
        onSuccess: (_data, { alertId, newStatus }) => {
            ...
            broadcastRefresh('eq:alert-updated', { projectId, alert_id: alertId, status: newStatus })
    ...
    return (
        <>
            <PreviewModal open={!!snapshotUrl} onClose={() => setSnapshotUrl(null)} title="Insight Evidence">
            ...
            <div className="card stretch stretch-full">
                <div className="card-header">
                    <div>
                        <h5 className="mb-0">Equipment Insights</h5>
                        <span className="fs-12 text-muted">Zone-level equipment activity and utilization insights across monitored cameras</span>
                    </div>
                </div>

                <div className="card-body p-0 eq-alerts-body d-flex flex-column">
                    {alerts.length === 0 && effectiveLoading ? (
                        <div className="text-center py-5 text-muted">Loading insights…</div>
                    ) : alerts.length === 0 ? (
                        ...
                        <div className="table-responsive eq-alerts-responsive">
            ...
            <style>{`
                ...
                .eq-alerts-responsive {
                    transition: none !important;
                    overflow: visible !important;
                }
                .eq-alerts-responsive table,
                .eq-alerts-responsive tbody {
                    transition: none !important;
                }
                .eq-alerts-responsive table {
                    table-layout: fixed;
                }
                .eq-alerts-responsive th:first-child,
                .eq-alerts-responsive td:first-child {
                    padding-left: 15px !important;
                }
                .eq-alerts-responsive th:last-child,
                .eq-alerts-responsive td:last-child {
                ...
                .pm-modal-iconbtn {
                    ...
                    background: rgba(0,0,0,0.58); color: rgba(255,255,255,0.96);
                    display: inline-flex; align-items: center; justify-content: center;
                    padding: 0; cursor: pointer; transition: all 140ms ease;
                    box-shadow: 0 8px 18px rgba(0,0,0,0.35); backdrop-filter: blur(6px);
                }
                .pm-modal-iconbtn:hover { background: rgba(0,0,0,0.74); border-color: rgba(255,255,255,0.44); color: rgba(255,255,255,0.98); box-shadow: 0 10px 22px rgba(0,0,0,0.42); transform: translateY(-2px); }
                .pm-modal-iconbtn:active { background: rgba(0,0,0,0.82); transform: translateY(0px); box-shadow: 0 6px 14px rgba(0,0,0,0.30); }
                html.app-skin-dark .pm-modal-iconbtn { background: rgba(0,0,0,0.58); border-color: rgba(255,255,255,0.34); color: rgba(255,255,255,0.96); box-shadow: 0 8px 18px rgba(0,0,0,0.35); }
                html.app-skin-dark .pm-modal-iconbtn:hover { background: rgba(0,0,0,0.74); border-color: rgba(255,255,255,0.44); color: rgba(255,255,255,0.98); box-shadow: 0 10px 22px rgba(0,0,0,0.42); transform: translateY(-2px); }
                html.app-skin-dark .pm-modal-iconbtn:active { background: rgba(0,0,0,0.82); transform: translateY(0px); box-shadow: 0 6px 14px rgba(0,0,0,0.30); }
                .pm-zoom-wrap { width: 100%; position: relative; min-height: 420px; }
                .pm-media-loading { width: 100%; min-height: 420px; display: flex; flex-direction: column; align-items: center; justify-content: center; border-radius: 12px; background: rgba(2,6,23,0.04); }
                ...
                .pm-zoom-stage { width: 100%; height: auto; max-height: 70vh; display: flex; align-items: center; justify-content: center; overflow: hidden; border-radius: 12px; background: transparent; border: 0; touch-action: none; }
                .pm-zoom-img { max-width: 100%; max-height: 70vh; object-fit: contain; border-radius: 10px; user-select: none; -webkit-user-drag: none; will-change: transform; transition: transform 120ms ease-out; }
                .pm-zoom-controls { position: absolute; right: 10px; bottom: 18px; display: flex; gap: 8px; }
