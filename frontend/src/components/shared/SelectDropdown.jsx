...
const SelectDropdown = ({ options, selectedOption, onSelectOption, className, defaultSelect, hideSearch, staticArrow = false }) => {
        ...
        <div className={`select-dropdown ${className ? className : ""} ${openUpwards ? 'open-upwards' : ''}`} ref={dropdownRef}>
            <div className={"select-box"} onClick={toggleDropdown}>
                <span className="selected-label">
                    ...
                    {localSelectedOption?.icon ? <span className={`d-inline-flex align-items-center fs-16 ${localSelectedOption.iconClassName}`}>{getIcon(localSelectedOption?.icon)} </span> : ""}
                    {localSelectedOption?.img ? <img src={localSelectedOption.img} className="avatar-image avatar-sm" /> : ""}
                    {localSelectedOption?.label}
                </span>
                <span className="arrow">{staticArrow ? <FiChevronDown /> : (isOpen ? <FiChevronUp /> : <FiChevronDown />)}</span>
            </div>
                        ...
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map((option) => (
                                ...
                                >
                                    ...
                                    {option?.icon ? <span className={`d-inline-flex align-items-center me-3 fs-16 ${option.iconClassName}`}>{getIcon(option?.icon)}</span> : ""}
