...
import { validateHumanText } from '@/utils/projectValidation'
...
const AddTask = () => {
    ...
    const validateForm = () => {
        const plainNote = value.replace(/<[^>]*>/g, ' ').replace(/&nbsp;/g, ' ')
        const errs = {}
        const nameErr = validateHumanText(taskName, 'Task name', { min: 2, max: 500 })
        if (nameErr) errs.taskName = nameErr
        const noteErr = validateHumanText(plainNote, 'Description', {
            required: false,
            min: 5,
            max: 2000,
            multiline: true,
        })
        if (noteErr) errs.description = noteErr
        setFieldErrors(errs)
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setFieldErrors(prev => ({ ...prev, taskName: null }))
                                }}
                            />
                            {fieldErrors.taskName && (
                                <div className="text-danger fs-11 mt-1">{fieldErrors.taskName}</div>
                            )}
